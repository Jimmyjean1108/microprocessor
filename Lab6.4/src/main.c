...
void blink1(){
	set_gpio(LED_gpio, LED1_pin);
	delay_without_interrupt(500);
	reset_gpio(LED_gpio, LED1_pin);
	delay_without_interrupt(500);
}
void blink2(){
	set_gpio(LED_gpio, LED2_pin);
	delay_without_interrupt(500);
	reset_gpio(LED_gpio, LED2_pin);
	delay_without_interrupt(500);
}

void ring(){
	timer_enable(TIM2);
	timer_init(TIM2, 200000/D, 19);
	delay_without_interrupt(400);
}

void toggle_output(){
	display_number(SEG_gpio, DIN_pin, CS_pin, CLK_pin, a, num_digits(a));
	a++;
	if((a-1)>=10){
		flag=1;
		a=0;
...
int main(){
	...
	init_led(LED_gpio, LED1_pin);
	init_led(LED_gpio, LED2_pin);
	//EXTI_Setup();
	EXTI_Setup();
	...
	send_7seg(SEG_gpio, DIN_pin, CS_pin, CLK_pin, SEG_ADDRESS_SCAN_LIMIT, 0x01);
	//Wakeup 7seg
	...
	SystemClock_Config_interrupt(10,30000000);
